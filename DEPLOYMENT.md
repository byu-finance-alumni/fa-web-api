...
| `OPPORTUNITY_LINK_NOTIFY_MODE` | `per_posting` (default — one alert per survey job posting, immediately) or `daily_digest` (one summary a day). Optional; anything unrecognised means `per_posting`, so a typo cannot silence the feature. ⚠️ `daily_digest` **also requires** adding `{"path": "/opportunity-links/cron/digest", "schedule": "0 17 * * *"}` to `vercel.json` — the mode alone moves the message off the request path and nothing else sends it. |
