...
| `SLACK_SUBMISSION_WEBHOOK_URL` | Slack **incoming-webhook** URL for **submission** alerts — an alumnus sent us something a human must action; today that is a job posting arriving through the survey (#771). Optional. **Same one-direction fallback:** unset here with `SLACK_ALERT_WEBHOOK_URL` set sends postings to `#error-alerts` rather than dropping them. Messages are tagged `POSTING`. |
| ~~`OPPORTUNITY_LINK_NOTIFY_MODE`~~ | **Retired (#567) — delete it if it is set.** How survey job postings are announced is now decided by the digest recipient list in the engineer console (Maintenance page): recipients set ⇒ one e-mail to them at 6pm Mountain on days a posting arrived (the two `/opportunity-links/cron/digest` entries in `vercel.json`); none set ⇒ one alert per posting on the channels above. The digest uses `RESEND_API_KEY` / `SURVEY_FROM_EMAIL` and every digest e-mail is counted in the survey's daily and monthly send budget. |
