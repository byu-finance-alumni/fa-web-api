...
# --- events: attendees into an EXISTING event (#611) --------------------------
#
# The reverse order of the wizard: the event was created on its own (no roster),
# and the CSV is uploaded to it later. Nothing about the event identity is read
# from the file, and the event itself is never created or replaced.


def _existing_event(event_id=7, name="Banquet", date="2026-04-15"):
    return SimpleNamespace(
        event_id=event_id,
        event_name=name,
        event_date=datetime.date.fromisoformat(date) if date else None,
    )


def test_events_attendee_import_adds_matched_new_rows():
    rows, _ = import_events.parse_and_map(
        _bytes(_EV_HEADER + "jdoe,Jane,Doe,VIP\nghost,No,Body,\n")
    )
    # execute() #1 = Net-ID matches, #2 = the event's current roster (empty).
    session = _SeqSession([[(42, "jdoe")], []])
    report = asyncio.run(
        import_events.evaluate_for_event(session, rows, _existing_event())
    )
    assert report["columns_ok"] is True
    assert report["event"]["event_id"] == 7
    assert report["importable"] is True
    assert report["summary"]["attendees_new"] == 1
    assert report["summary"]["attendees_unmatched"] == 1
    jane = next(a for a in report["attendees"] if a["net_id"] == "jdoe")
    assert jane["alumni_id"] == 42
    assert jane["already_attending"] is False
    assert jane["notes"] == "VIP"


def test_events_attendee_import_skips_people_already_on_the_roster():
    rows, _ = import_events.parse_and_map(
        _bytes(_EV_HEADER + "jdoe,Jane,Doe,\nmsmith,Mark,Smith,\n")
    )
    # Jane (42) already attends; Mark (43) is new.
    session = _SeqSession([[(42, "jdoe"), (43, "msmith")], [(42,)]])
    report = asyncio.run(
        import_events.evaluate_for_event(session, rows, _existing_event())
    )
    assert report["summary"]["attendees_matched"] == 2
    assert report["summary"]["attendees_existing"] == 1
    assert report["summary"]["attendees_new"] == 1
    assert report["importable"] is True
    jane = next(a for a in report["attendees"] if a["net_id"] == "jdoe")
    assert jane["already_attending"] is True


def test_events_attendee_import_is_a_no_op_when_everyone_already_attends():
    rows, _ = import_events.parse_and_map(_bytes(_EV_HEADER + "jdoe,Jane,Doe,\n"))
    session = _SeqSession([[(42, "jdoe")], [(42,)]])
    report = asyncio.run(
        import_events.evaluate_for_event(session, rows, _existing_event())
    )
    # Nothing to add — say so rather than pretend to import.
    assert report["importable"] is False
    assert any(w["code"] == "all_already_attending" for w in report["warnings"])


def test_events_attendee_import_warns_when_nothing_matched():
    rows, _ = import_events.parse_and_map(_bytes(_EV_HEADER + "ghost,No,Body,\n"))
    session = _SeqSession([[], []])
    report = asyncio.run(
        import_events.evaluate_for_event(session, rows, _existing_event())
    )
...
def test_events_attendee_import_bad_headers_preview_keeps_the_event():
    report = import_events.headers_bad_attendee_preview(
        ["Missing required column: 'Net ID'."], _existing_event()
    )
    assert report["columns_ok"] is False
    assert report["importable"] is False
    assert report["event"]["event_id"] == 7
    assert report["summary"]["attendees_new"] == 0


class _AttendeeCommitSession(_SeqSession):
    """``_SeqSession`` plus the write surface ``commit_attendees_for_event``
    touches. Records every added object so the test can assert the attendance
    rows and audit entries — and that no Event was ever constructed."""
...
def test_events_attendee_commit_adds_only_new_and_never_touches_the_event():
    rows, _ = import_events.parse_and_map(
        _bytes(_EV_HEADER + "jdoe,Jane,Doe,VIP\nmsmith,Mark,Smith,\nghost,No,Body,\n")
    )
    # Jane (42) already attends, Mark (43) is new, ghost is unmatched.
    session = _AttendeeCommitSession([[(42, "jdoe"), (43, "msmith")], [(42,)]])
    result = asyncio.run(
        import_events.commit_attendees_for_event(
            session, rows, _existing_event(), actor_user_id=9
        )
    )
    ...
    assert [u["net_id"] for u in result["unmatched"]] == ["ghost"]
    assert session.committed is True

    # Exactly one attendance row (Mark) + its audit entry, and NO Event object:
    # the import adds to the event, it never creates or replaces one.
    attendance = [o for o in session.added if isinstance(o, EventAttendance)]
    assert len(attendance) == 1
    assert attendance[0].alumni_id == 43
    ...
    assert [a.action_type for a in audits] == ["add_attendee"]
    assert audits[0].entity_type == "event"
    assert audits[0].entity_id == 7


def test_events_attendee_commit_writes_nothing_when_there_is_nothing_new():
    rows, _ = import_events.parse_and_map(_bytes(_EV_HEADER + "jdoe,Jane,Doe,\n"))
    session = _AttendeeCommitSession([[(42, "jdoe")], [(42,)]])
    result = asyncio.run(
        import_events.commit_attendees_for_event(session, rows, _existing_event())
    )
    ...
    assert "already an attendee" in result["error"]
...
# --- routes: attendees into an EXISTING event (#611) --------------------------
...
class _EventAttendeeImportSession(_AttendeeCommitSession):
    """Commit-capable fake that also answers ``session.get(Event, id)``."""

    def __init__(self, results, event=None):
        super().__init__(results)
        self._event = event
...
def test_attendee_import_requires_auth(client):
    assert (
        client.post("/events/7/attendees/import", files=_ATTENDEE_CSV).status_code
        == 401
    )


def test_attendee_import_forbidden_for_view_only(client):
    app.dependency_overrides[get_current_db_user] = lambda: _ctx("view_only")
    assert (
        client.post("/events/7/attendees/import", files=_ATTENDEE_CSV).status_code
        == 403
...
def test_attendee_import_preview_forbidden_for_view_only(client):
    app.dependency_overrides[get_current_db_user] = lambda: _ctx("view_only")
    assert (
        client.post(
            "/events/7/attendees/import/preview", files=_ATTENDEE_CSV
        ).status_code
        == 403
    )


def test_attendee_import_unknown_event_is_404(client):
    app.dependency_overrides[get_current_db_user] = lambda: _ctx("full_access")

    async def _override():
        yield _EventAttendeeImportSession([], event=None)

    app.dependency_overrides[get_session] = _override
    response = client.post("/events/7/attendees/import", files=_ATTENDEE_CSV)
    assert response.status_code == 404


def test_attendee_import_preview_happy_path(client):
    app.dependency_overrides[get_current_db_user] = lambda: _ctx("full_access")

    async def _override():
        yield _EventAttendeeImportSession(
            [[(42, "jdoe")], []], event=_existing_event()
        )
    ...
    assert body["importable"] is True
    assert body["event"]["event_id"] == 7
    assert body["summary"]["attendees_new"] == 1


def test_attendee_import_commit_adds_to_the_existing_event(client):
    app.dependency_overrides[get_current_db_user] = lambda: _ctx("full_access")
    session = _EventAttendeeImportSession([[(42, "jdoe")], []], event=_existing_event())
    ...
    assert body["added"] == 1
    # The event itself is untouched — only an attendance row was written.
    assert not [o for o in session.added if isinstance(o, Event)]
    assert len([o for o in session.added if isinstance(o, EventAttendance)]) == 1


def test_attendee_import_bad_headers_import_nothing(client):
    app.dependency_overrides[get_current_db_user] = lambda: _ctx("full_access")
    session = _EventAttendeeImportSession([], event=_existing_event())
